...
while(1):
	...
	for i in range(cropped_x_start, cropped_x_stop-1):
		for j in range(cropped_y_stop-1, cropped_y_start, -1):
			if (image_cue[j,i][1]) > THRESHOLD_VAL: # if detect object, based on green
				image_cue[1,i][0] = 255 # blue
				image_cue[3,i][2] = 0 # clear the ID, if noise happens
				image_cue[5,i][2] = 0 # clear the ID, if noise happens
				break
			...
			# apply identifier
				...
				if (block_start < gate_left) and (block_end > gate_left):
					if (image_prev[3, int((block_start + block_end)/2) ][2] != 0) :
						...
						#if (image_prev[3, int((block_start + block_end)/2) ][2] == 0):
						#print("{} {} {} {} leftedge".format(vehicle_id, framenum, block_start, block_end))
					else:
				...
				# from right gate
					...
					if (image_prev[5, int((block_start + block_end)/2) ][2] != 0) :
						vehicle_id= image_prev[5, int((block_start + block_end)/2) ][2]
						#if (image_prev[2, int((block_start + block_end)/2) ][2] == 0):
						#print("{} {} {} {} rightedge {}".format(vehicle_id, framenum, block_start, block_end, gate_right))
					else:
						vehicle_id= random.randint(101, 199)
						#if (image_prev[2, int((block_start + block_end)/2) ][2] == 0):
						#print("{} {} {} {} rightnew {}".format(vehicle_id, framenum, block_start, block_end, gate_right))
					for n in range(block_start, block_end):
						image_cue[5,n][2] = vehicle_id
	...
	for j in range(cropped_y_start, cropped_y_stop):
		image_display[j,gate_left][1] = 255 # green
		image_display[j,gate_right][1] = 255 # green
